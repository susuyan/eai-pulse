...
- 具身数据发展脉络覆盖 2022-01-31 至 2026-09-20，以 6 个连续阶段 × 六段生产管线组织 42 个已发布 Event；阶段与趋势只提供解释，原始证据继续经 Event 回链。缺少证据的影响单元明确标记 no-public-evidence，不推断补齐。
- 具身数据首页新增上一阶段转折与当前策展阶段摘要，保留策展截止日和最新支撑事件日期，并可直达完整发展脉络。
- 具身数据首页呈现 8 个跨 Event 主题趋势，按 Asia/Shanghai 日历日期生成当日稳定的阅读顺序；日期轮换只改变发现顺序，不代表新事实或实测热度。
- 具身数据阶段、六段影响、趋势与完整事件索引均预渲染为静态 HTML；无 JavaScript 时保留全部内容和策展顺序，启用脚本后的管线筛选也不隐藏完整事件索引。
- 公开产品已从通用 AI 行业信息流切换为具身数据生产全链路认知系统，围绕关键变化、六阶段管线、数据资产、同行能力、来源覆盖与行动建议六个视图组织可回链 Event；已补齐 2025-04 至 2026-09 的已核验事件与原始证据。
- 修复公开快照落后于具身数据 Event catalog 的问题，将 2025-04 至 2026-08 的 6 条已核验补录写入 Pages 发布输入；回归门禁要求 catalog 中的每条公开 Event 都存在于版本化快照。
- 行动建议改为由触发 Event 的事实、技术判断、行业影响、后续观察和建议动作共同生成；六类卡片分别给出目标角色、实验产物和失效条件，页面同时展示建议产物与依据事实，旧通用类型不再穿透公开查询。
- 当前公开数据包含 12 个 Dataset、4 个 Standard、6 个 CollectionMethod 和 15 个同行能力档案；事实、推断、能力声明与待验证机会继续分层表达。
- 当前具身数据目录收录 83 个来源；公开地图为非退休条目展示接入、待接入、受限或替代状态，以及六阶段覆盖和计算出的覆盖缺口。这些目录状态不等同于已完成重点 adapter 接入，也没有来源被描述为 `active` 生产采集器。SQLite 仍是零配置默认数据库，尚未通过真实 MySQL 集成验证，因此不宣称 MySQL 兼容。
- 修复历史快照覆盖当前来源范围与生命周期的问题，恢复 Figure AI 在具身数据地图中的展示；手动来源保留已审核的待接入或替代状态，健康状态仍为未检查。目录写入前强制校验地图状态、非空阶段覆盖、替代引用和限制说明。
...
- 修复 Source Audit 只更新来源生命周期快照、未同步系统评测基线的问题；审计现在在远端快照合并与来源对账后重新评测，并将来源健康报告、评测报告和快照原子提交，避免后续 CI 将仓库自身的数据漂移误判为代码回退。
...
- 评测门禁补齐严格时间、历史上下文与运营基线校验，时区等价时间与非法日期不再扭曲评分，无效 baseline 也不会被 Data Refresh 原地覆盖；Quality Guard 与 Monitor 使用同一当前分数权威和告警指纹，并保留共享 Issue 中尚未复核的站点/来源故障；任意未完成刷新均阻止重复派发，恢复刷新在周日也不生成周报。

...
import { localize, pageHero, relationLinks, statusChip, stringList } from "./shared.js";

export function renderAssetsPage(model: StaticSiteModel, locale: Locale): string {
  const datasets = model.datasets
    .map(
      (item) =>
        `<article class="asset-card" id="${escapeHtml(item.slug)}"><header><h2>${escapeHtml(item.name)}</h2>${statusChip(item.evidenceStatus, locale)}</header><p>${escapeHtml(item.publisher)}</p><a href="${escapeHtml(item.canonicalUrl)}" target="_blank" rel="noopener noreferrer">${escapeHtml(localize(locale, "官方入口", "Official page"))}</a><h3>${escapeHtml(localize(locale, "采集方式", "Acquisition"))}</h3>${stringList(item.acquisitionMethods)}<h3>${escapeHtml(localize(locale, "已知边界", "Known limitations"))}</h3>${stringList(item.limitations)}<h3>${escapeHtml(localize(locale, "相关事件", "Related events"))}</h3>${relationLinks(item.relatedEvents, "events/", locale)}</article>`,
    )
    .join("");
  const standards = model.standards
    .map(
      (item) =>
        `<article class="asset-card" id="${escapeHtml(item.slug)}"><header><h2>${escapeHtml(item.name)}</h2>${statusChip(item.evidenceStatus, locale)}</header><p>${escapeHtml(item.organization)} · ${escapeHtml(item.standardType)}</p><a href="${escapeHtml(item.canonicalUrl)}" target="_blank" rel="noopener noreferrer">${escapeHtml(localize(locale, "标准原文", "Standard source"))}</a><h3>${escapeHtml(localize(locale, "覆盖领域", "Coverage"))}</h3>${stringList(item.areas)}<h3>${escapeHtml(localize(locale, "相关事件", "Related events"))}</h3>${relationLinks(item.relatedEvents, "events/", locale)}</article>`,
    )
    .join("");
  const methods = model.collectionMethods
    .map(
      (item) =>
        `<article class="asset-card" id="${escapeHtml(item.slug)}"><header><h2>${escapeHtml(item.name)}</h2>${statusChip(item.evidenceStatus, locale)}</header><p>${escapeHtml(item.methodKind)} · ${escapeHtml(item.maturity)}</p><a href="${escapeHtml(item.canonicalUrl)}" target="_blank" rel="noopener noreferrer">${escapeHtml(localize(locale, "方法依据", "Method evidence"))}</a><h3>${escapeHtml(localize(locale, "质量边界", "Quality boundaries"))}</h3>${stringList(item.qualityBoundaries)}<h3>${escapeHtml(localize(locale, "失败模式", "Failure modes"))}</h3>${stringList(item.failureModes)}<h3>${escapeHtml(localize(locale, "相关事件", "Related events"))}</h3>${relationLinks(item.relatedEvents, "events/", locale)}</article>`,
    )
    .join("");

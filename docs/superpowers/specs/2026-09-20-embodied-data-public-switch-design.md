...
本次领域切换会主动退役通用 AI 生产来源并将 36 个具身来源保持在 `shadow`，因此旧领域的来源覆盖、稳定性和时效性加权分不能作为新领域的同口径回归基线。CI 只允许一次显式的评测域迁移：候选基线 Git SHA 与候选基线 snapshot hash 必须同时匹配 `embodied-data-public-switch-baseline.json`，当前 snapshot hash 必须匹配公开切换验收报告，并由当前时间的独立评测证明具身质量门禁全部通过且通用 AI 泄漏为 0。任一条件不满足时仍按普通回归失败；切换完成后的提交恢复与前一份具身运营基线比较。新的运营基线只能由合并后的真实 Data Refresh 持久化。

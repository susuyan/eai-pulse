...
- 审计 job 创建时即在私有 details_json 保存已解析且去重的 targetSourceIds、expectedSourceCount 与 auditComplete=false；指定来源集合及全部来源审计采用同一规则。完成时保留目标集合并原子更新完成标记。目标成员不进入公开 DTO 或仓库 snapshot。
- 门禁独立读取目标包含当前来源的 source-audit jobs，不能只通过已有 SourceCheck 发现作业。缺失目标检查、running、failed、AUDIT_INCOMPLETE 或不完整 partial 都截断健康区间；即使标记 succeeded 也必须具有全部目标成员的持久化检查。运行中的目标作业阻断当前决定；无法定位时间的目标作业拒绝。完整失败结束后可重新积累三个满足间隔的健康检查。
- 检查要求 job 的明确目标集合、声明数量、实际持久化检查成员及状态计数完全一致；partial 仅允许其他已持久化来源失败。旧 job 没有目标集合时不能贡献 priority 证据；不从检查反推或补造原目标集合。
